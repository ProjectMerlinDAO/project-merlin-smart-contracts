...
async function main() {
  ...
  // Deploy TokenManager first with minimal bridge amount
  // This is separate from the bridge deployment which will have larger allocations
  console.log("Deploying minimal TokenManager for DAO functionality...");
  const TokenManager = await ethers.getContractFactory("TokenManager");
  const tokenManager = await TokenManager.deploy(
    "Merlin",                       // name
    "MRLN",                         // symbol
    ethers.parseEther("1000000"),   // minimal totalSupply for DAO operations
    ethers.parseEther("10000"),     // minimal bridgeAmount
    100,                            // transferFee (1%)
  ...
  // Get Bridge and Oracle addresses from TokenManager
